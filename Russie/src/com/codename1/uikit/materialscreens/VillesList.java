...
public class VillesList extends SideMenuBaseForm
{
    Form f;
    String url2 = "/img";
    ...
    TextField tfNom;
    String prenom;
    public VillesList(Resources res)  
    {
        created = Database.exists("Russia");
        try 
        {
           db = Database.openOrCreate("Russia");
                if(created == true) db.execute("create table commentville (id INTEGER PRIMARY KEY AUTOINCREMENT, user TEXT, body TEXT, datec TEXT, idville INTEGER);");
                System.out.println("Table commentville was created.");
 ...
 System.out.println("Path of database: "+Database.getDatabasePath("Russia"));
        ...
        Label tit = new Label("Villes", "Title");
...
//gov.getAllStyles().setMargin(LEFT, 0);
        ...
        ArrayList<Villes> p = sp.getList2();
        ...
        for (Villes pr : p) {
            ...
            {
                List<String> latlong = StringUtil.tokenize(pr.getCoordonnees(), ",");
        ...
        System.out.println("coordinates: "+latitude+" "+longitude);
            ...
            URLImage imgsv = URLImage.createToStorage(enc, url2+pr.getPhotoville(), url+pr.getPhotoville());
                ImageViewer imgvb = new ImageViewer(imgsv);
                Label capa = new Label("Capacité: "+pr.getPopulation());
                Label inau = new Label("Fondation: "+pr.getFondation());
                Label equipe = new Label("Equipe locale: "+pr.getEquipelocale());
                Label timeLabel = new Label("Timezone: "+pr.getTimezone());
            ...
            public void actionPerformed(ActionEvent e) 
            {
                    ...
                    public void actionPerformed(ActionEvent evt) {
                        ...
                        try {
                            if (isInputValid())
                            {
                                ...
                                db.execute("insert into commentville (user, body, datec, idville) values ('"+u.getUsername()+"', '"+nom+"', '"+str+"','"+pr.getId()+"' );");
                                ...
                                f2.revalidate();
                                VillesList cl = new VillesList(res);
                                cl.getF().show();
                            }
                        } catch (IOException ex)
                        {
                ...
                try {
                    Image placeholder = Image.createImage("/cmnt.png");
                        ...
                        f2.add(title);
                    Cursor c= db.executeQuery("select * from commentville where idville ="+pr.getId());
                   while (c.next()) 
                   {
                        Row r = c.getRow();
                    ...
                    public void actionPerformed(ActionEvent evt) {
                        ...
                        try {
                            if (isInputValid())
                            {
                                ...
                                Date dt = new Date();
                                String str = new SimpleDateFormat("dd/MM/yyyy : HH:mm").format(dt);
                                db.execute("UPDATE commentville SET body ='"+nom+"',datec ='"+str+"' WHERE idville='" + pr.getId() + "' AND user='" + u.getUsername() + "' ");
                                System.out.println(pr.getId());
                                System.out.println(pr.getNom());
                                System.out.println(u.getId());
                                System.out.println(u.getUsername());
                        ...
                        public void actionPerformed(ActionEvent evt) 
                        {
                            try 
                            {
                                ...
                                db.execute("DELETE from commentville WHERE idville='" + pr.getId() + "' AND user='" + u.getUsername() + "' ");
        ...
        f.show();
                        ...
                        System.out.println(pr.getPhotoville());
                        int deviceWidth = Display.getInstance().getDisplayWidth() / 4;
                                ...
                                label.setIcon(URLImage.createToStorage(encImage,
                                        "Large_" + url2 +pr.getPhotoville()+
                                                "", url +pr.getPhotoville()+
